...
import {
    saveConsultation, fetchConsultations, convertConsultationStatus, deleteConsultation,
    ConsultationData
...
// --- TYPE DEFINITIONS FOR NEW EVENT STRUCTURE ---
// Matches the Firebase JSON structure provided
interface UnifiedEventData {
    ...
    customer: {
        ...
        mobile: string;
        contacts: any[];
        verification?: { type: string; documentNumber: string };
    };
    schedule: {
        dayCount: number;
        days: Array<{
    ...
    approval: { customerConfirmed: boolean; confirmedAt?: Date; customerEmail: string; customerPhone: string };
...
export default function ConsultationPage() {
    ...
    // --- CUSTOM ITEM HANDLERS (Matches EventForm Logic) ---
    ...
    const handleLoadDrafts = async () => {
        ...
        setDraftPage(1); // Reset to page 1
        try {
    ...
    // --- CONVERSION LOGIC (MAPPING) ---
    const mapConsultationToEvent = (): UnifiedEventData => {
        const now = new Date();

        // Determine Package Cost vs Custom Cost
        ...
        if (!isCustomPackage && consultation.package.selectedPackageId) {
            const pkg = packages.find(p => p.id === consultation.package.selectedPackageId);
            if (pkg) {
        ...
        // Map CustomItems to "additionalServices" format for Budget
        const additionalServices = consultation.package.customItems.map(item => ({
        ...
        return {
            ...
            schedule: {
                dayCount: 1, // Defaulting to 1 day for consultation conversion
                days: consultation.requirements.date ? [{
                    date: consultation.requirements.date,
                    type: isCustomPackage ? 'custom' : 'package',
                    packageId: pkgId,
                    cost: isCustomPackage ? customTotal : packageCost, // Day cost
                    customItems: [] // If items are per-day specific, they go here. For global, we use additionalServices
                }] : [],
                locations: []
            },
            budget: {
    ...
    const handleConvertToEvent = async () => {
        ...
        setLoading(true);
        try {
            // 1. Map Data to New Structure
            const eventPayload = mapConsultationToEvent();

            // 2. Create Event (Typescript cast needed if service expects legacy type, 
            // assuming createEvent handles the new JSON structure on backend)
            await createEvent(userData.studioID, eventPayload as unknown as EventData);
                        ...
                        <div className="p-6 pb-24 max-w-6xl mx-auto w-full">
                                        ...
                                        <CardContent className="p-8 space-y-8">
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                ...
                                                <div className="space-y-2 md:col-span-2">
                                                    ...
                                                    <Input value={consultation.client.email} onChange={e => setConsultation({ ...consultation, client: { ...consultation.client, email: e.target.value } })} className="h-12 text-lg bg-slate-50" placeholder="email@example.com" />
                                                </div>
                                            </div>
                                            <Separator />
                            ...
                            {step === 3 && (
                                <div className="space-y-6 animate-in slide-in-from-right-4 fade-in duration-300">
                                    ...
                                    {/* 3. Custom Add-ons Builder (UI like EventForm) */}
                                    <div className="bg-white border border-slate-200 p-6 rounded-xl shadow-sm space-y-6">
                                        <div className="flex justify-between items-center border-b pb-4">
                                            <h3 className="font-bold text-slate-800 text-lg flex items-center gap-2">
                                                {consultation.package.selectedPackageId === 'custom' ? <><LayoutTemplate className="w-5 h-5 text-blue-600" /> Custom Package Items</> : <><Plus className="w-5 h-5 text-blue-600" /> Add-ons & Extras</>}
                                            </h3>
                                        ...
                                        {consultation.package.customItems.length > 0 ? (
                                            <div className="space-y-3">
                                                ...
                                                <div className="grid grid-cols-12 gap-4 text-xs font-semibold text-slate-500 uppercase tracking-wider px-2">
                                                    ...
                                                    <div className="col-span-4 text-right">Price (LKR)</div>
                                                ...
                                                <div className="flex justify-end pt-4 border-t">
                                                    <div className="text-right">
                                                        <p className="text-xs text-slate-500 uppercase">
                                                            {consultation.package.selectedPackageId === 'custom' ? "Custom Total" : "Add-ons Total"}
                                                        </p>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        <Card>
                                            ...
                                            <CardContent className="space-y-4 pt-6 text-sm">
                                                ...
                                                <div className="flex justify-between py-2 border-b"><span className="text-slate-500">Date</span><span className="font-medium">{consultation.requirements.date ? format(consultation.requirements.date, "PPP") : "TBD"}</span></div>
                                            </CardContent>
                                        </Card>
                    ...
                    <div className="min-h-[300px] flex flex-col justify-between">
                        <div className="space-y-2">
                            {loadingDrafts ? <div className="p-4 text-center"><Loader2 className="w-6 h-6 animate-spin mx-auto" /></div> : (
                                paginatedDrafts.length === 0 ? <p className="text-center text-slate-400 py-4">No drafts found.</p> :
                                    paginatedDrafts.map(draft => (
                                        <div key={draft.id} className="group p-3 border rounded hover:bg-slate-50 flex justify-between items-center cursor-pointer" onClick={() => selectDraft(draft)}>
                                            <div>
                                                <div className="font-medium">{draft.client.name || "Untitled Draft"}</div>
                                                <div className="text-xs text-slate-500 flex gap-2">
                                                    <span>{draft.requirements.eventType}</span>
                                                    <span>•</span>
                                                    <span>{draft.updatedAt ? format(safeDate(draft.updatedAt), "MMM dd") : ""}</span>
                                                </div>
                                            </div>
                                            <Button
                                                variant="ghost"
                            ...
                            <div className="mt-4 flex justify-center">
                                <Pagination>
                                    <PaginationContent>
                                        ...
                                        <PaginationItem>
                                            <div className="text-sm px-4 py-2">Page {draftPage} of {totalPages}</div>
                                        </PaginationItem>
                                        <PaginationItem>
